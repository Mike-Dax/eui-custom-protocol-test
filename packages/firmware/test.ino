...
#define FRAMING_BYTE_SUFFIX (0xFD)   // 0b01010101
...
void user_callback( void )
{
  ...
  {
    // CMD_PULSE_AMP_SET
    if( decoder.packet.command == 0x83 )
    {
    ...
    // CMD_PULSE_AMP_RD
    // if( decoder.packet.command == 0x13 )
    // {
    //   // Just ignore the payload
    //   
    //   uint8_t led_status = digitalRead( LED_BUILTIN ); // Read the LED state
    // 
    //   // Is this sent to 0x00 as the 'master'?
    //   //    Change addr to `decoder.packet.address` as needed
    //   Packet_t response = { .address = 0x00, .command = 0x02, .payload = led_status };
    //   encode( &response );  
    // }
