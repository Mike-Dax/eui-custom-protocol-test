...
describe("Framing Decoder", () => {
  ...
  it(
    "handles multiple packets with garbage between them",
    generateDecoderEqualityTest(
      Buffer.from([
        0xfe, 0x02, 0x83, 0x88, 0xfd, 0x03, 0x03, 0xff, 0x00, 0xfe, 0x8e, 0x8e,
        0x8e, 0xfd,
      ]),
      [
        Buffer.from([0xfe, 0x02, 0x83, 0x88, 0xfd]),
        Buffer.from([0xfe, 0x8e, 0x8e, 0x8e, 0xfd]),
      ]
    )
  );
  it(
    "does nothing with garbage data",
    generateDecoderEqualityTest(
      Buffer.from([
        0x02, 0xab, 0xde, 0xaf, 0xbe, 0xee, 0xee, 0xee, 0xee, 0xee, 0xef,
      ]),
      []
    )
  );
